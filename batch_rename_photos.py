...
programs = ["1. Main Features", "2. After Hours", "3. Special Screenings",
            "4. Family Day on Aragon", "5. See It In 70mm",
...
# counter used for file number suffix, e.g., file 1, file 2, etc.
num_suffix = 1

# variable containing final path to files; the first argument will never change
# as this is the highest-level common directory.
# Coral Gables Art Cinema/Programming/Main Features/Top Five
base_path = os.path.join("M:/Coral Gables Art Cinema/Programming/",
                         prog_dir[3:], year_dir, mov_dir, photo_dir)

# renaming occurs here
for photo in os.listdir(base_path):
    if photo.endswith('.jpg', '.jpeg', '.png')
    f_name = d_name + '' + str(num_suffix) + '.jpg'
